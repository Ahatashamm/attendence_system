...
<?php
              ...
              <h3 class="text-center" style="color:blue;">The Millennium University</h3>
			   <?php
			   $over1=mysqli_query($con,"select DISTINCT `dept` from attendence08");
			   while($row=mysqli_fetch_array($over1))
			   {
				 $dept=$row['dept'];  
				  
			   }
			   $over2=mysqli_query($con,"select DISTINCT `semester` from attendence08");
			   ...
			   $over3=mysqli_query($con,"select DISTINCT `batch` from attendence08");
			  ...
			  <tbody>
			  <?php
			  $sn=mysqli_query($con,"select DISTINCT `studentid` from attendence08");
			  ...
			  <?php 
			  $total=0;
			   $at=mysqli_query($con,"select * from attendence08 where (course='AI'  and studentid='$ro[studentid]' and active='1')");
			 ...
			 <?php 
			    $ats=mysqli_query($con,"select * from attendence08 where (course='Robotics'  and studentid='$ro[studentid]' and active='1')");
