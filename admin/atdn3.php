...
<?php
              ...
              <h3 class="text-center" style="color:blue;">The Millennium University</h3>
			   <?php
			   $over1=mysqli_query($con,"select DISTINCT `dept` from attendence03");
			   while($row=mysqli_fetch_array($over1))
			   {
				 $dept=$row['dept'];  
				  
			   }
			   $over2=mysqli_query($con,"select DISTINCT `semester` from attendence03");
			   ...
			   $over3=mysqli_query($con,"select DISTINCT `batch` from attendence03");
			  ...
			  <tbody>
			  <?php
			  $sn=mysqli_query($con,"select DISTINCT `studentid` from attendence03");
			  ...
			  <?php 
			  $total=0;
			   $at=mysqli_query($con,"select * from attendence03 where (course='compiler'  and studentid='$ro[studentid]' and active='1')");
			 ...
			 <?php 
			    $ats=mysqli_query($con,"select * from attendence03 where (course='Java'  and studentid='$ro[studentid]' and active='1')");
